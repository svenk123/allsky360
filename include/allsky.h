...
static inline void *allsky_safe_realloc_copy(void *pointer, size_t size, void *from) {
    pointer = realloc(pointer, size);
...
static inline char *allsky_safe_strncpy(char *dst, const char *src, size_t size) {
    if (size > 0) {
	dst[size - 1] = '\0';
...
static inline void allsky_safe_strcpy(char *dest, const char *src, size_t dest_size) {
...
static inline void allsky_safe_strcat(char *dest, const char *src, size_t dest_size) {
    assert(dest != NULL && src != NULL);
...
static inline int allsky_safe_snprintf(char *buf, size_t buf_size, const char *fmt, ...) {
...
static inline size_t allsky_safe_fread(void *ptr, size_t size, size_t count, FILE *stream) {
...
static inline size_t allsky_safe_fwrite(const void *ptr, size_t size, size_t count, FILE *stream) {
    assert(ptr != NULL && stream != NULL);
...
static inline int allsky_safe_rename(const char *old_path, const char *new_path) {
    ...
    if (rename(old_path, new_path) == -1) {
        switch (errno) {
            ...
            case EISDIR:
                ...
                break;
            case ENOENT:
                fprintf(stderr, "%s: Source or target directory does not exist (%s -> %s)\n", __func__, old_path, new_path);
                break;
            case ENOTDIR:
            ...
            case EROFS:
                ...
                break;
            case EXDEV:
                fprintf(stderr, "%s: Source and target are on different filesystems (%s -> %s)\n", __func__, old_path, new_path);
                break;
            case ENOSPC:
                fprintf(stderr, "%s: No space left on device (%s -> %s)\n", __func__, old_path, new_path);
                break;
            case EEXIST:
                fprintf(stderr, "%s: Target file exists and cannot be overwritten (%s -> %s)\n", __func__, old_path, new_path);
                break;
            default:
