function isShowerActive(shower, now) {
  const [startMonth, startDay] = shower.date[0];
  const [endMonth, endDay] = shower.date[1];
  const year = now.getUTCFullYear();

  const startDate = new Date(Date.UTC(
    startMonth > endMonth ? year - 1 : year,
    startMonth - 1, startDay
  ));

  const endDate = new Date(Date.UTC(year, endMonth - 1, endDay, 23, 59, 59));
...
const meteorShowers = {
  ...
  "ACE": { name: "Alpja-Centaurids",               date: [[1, 28],[2,21]],   pos: [[200,-57],[225,-63]] },
  ...
  "SPE": { name: "September Eta-Perseids",       date: [[9,  5],[9,21]],   pos: [[ 43, 40],[ 59, 41]] },
  ...
  "EGE": { name: "Eta-Geminids",                 date: [[10,14],[10,27]],  pos: [[ 99, 27],[109, 27]] },
...
function getRadiantPosition(shower) {
  const [ra1, dec1] = shower.pos[0];
  const [ra2, dec2] = shower.pos[1];
